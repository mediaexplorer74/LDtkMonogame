...
namespace GameManager.Entities
{
    public class PlayerEntity
    {
        ...
        public void Update(float deltaTime, float totalTime)
        {
            ...
            int h = (keyboard.IsKeyDown(Keys.A) || keyboard.IsKeyDown(Keys.Left) ? -1 : 0) 
        ...
        void CollisionDetection(LDtkLevel level, float deltaTime)
        {
            ...
            Vector2 bottomRight = Vector2.Max(collider.BottomRight, 
                collider.BottomRight + (velocity * deltaTime));

            Point topLeftGrid = collisions.FromWorldToGridSpace(topleft);
            Point bottomRightGrid = collisions.FromWorldToGridSpace(bottomRight + 
                (Vector2.One * collisions.TileSize));
            ...
            for (int x = topLeftGrid.X; x < bottomRightGrid.X; x++)
            {
                for (int y = topLeftGrid.Y; y < bottomRightGrid.Y; y++)
                {
                    ...
                    {
                        tiles.Add(new Box(level.Position.ToVector2() 
                            + new Vector2(x * collisions.TileSize, y * collisions.TileSize), new Vector2(collisions.TileSize), Vector2.Zero));
                    }
                }
            }
